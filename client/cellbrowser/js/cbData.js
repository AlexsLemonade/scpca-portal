...
var cbUtil = (function() {
  ...
  my.loadFile = function(url, arrType, onDone, onProgress, otherInfo, start, end) {
    ...
    oReq.setRequestHeader("api-key", window.scpca.token)
    oReq.setRequestHeader("Authorization", window.scpca.clientToken)
  ...
  my.loadTsvFile = function(url, onDone, addInfo) {
    ...
    JQuery.ajax({
      ...
      success: function(tsvString) {
        console.log(tsvString)
        Papa.parse(tsvString, {
          delimiter: "\t",
          skipEmptyLines: true,
          fastMode: true,
          // download: true,
