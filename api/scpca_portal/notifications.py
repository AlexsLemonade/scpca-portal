...
def send_dataset_file_completed_email(
    dataset_id: str,
    sender: str = settings.TEST_EMAIL_SENDER,
    recipient: str = settings.TEST_EMAIL_RECIPIENT,
) -> None:
    ...
    SUBJECT = f"All files generated for {dataset_id}"
    BODY_TEXT = (
        "Hot off the presses:\n\n"
        f"All files have been generated for project {dataset_id}"
        + "\n\nLove!,\nThe ScPCA Portal Team"
    )
    BODY_HTML = render_to_string(TEMPLATE_FILE_PATH, context={"dataset_id": dataset_id})
