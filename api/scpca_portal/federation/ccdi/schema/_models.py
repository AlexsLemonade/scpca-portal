...
class CdeV1SampleTumorTissueMorphology(BaseModel):
    icd_o_3: Annotated[str, Field(description='The ICD-O-3 code.')]


class FieldsUnharmonized(BaseModel):
    pass


class ModelsFileMetadataChecksums(BaseModel):
    md5: Annotated[
        str | None,
        Field(
            description='**`caDSR CDE 11556150 v1.00`**\n\nThis metadata element is defined by the caDSR as "A 32-character hexadecimal\nnumber that is computed on a file.". No permissible values are defined for\nthis CDE.\n\nLink:\n<https://cadsr.cancer.gov/onedata/dmdirect/NIH/NCI/CO/CDEDD?filter=CDEDD.ITEM_ID=11556150%20and%20ver_nr=1>'
        ),
    ] = None


class ModelsGatewayAnonymousOrReference2(BaseModel):
    gateway: Annotated[str, Field(description='The reference to a [`Named`] gateway.')]
    kind: Literal['Reference']


class ModelsGatewayClosed1(BaseModel):
    status: Literal['IndefinitelyClosed']


class ModelsGatewayClosed2(BaseModel):
    available_at: Annotated[
        AwareDatetime | None,
        Field(
            description='If known, the ISO 8601 formatted, UTC-based date and time when the\nthe resource will become available.\n\nThis field is intended to indicate to the consumer that they should\nretry their request on or after the listed time to gain an updated\ngateway definition.'
        ),
    ] = None
    status: Literal['AwaitingPublication']


class ModelsGatewayClosed3(BaseModel):
    available_at: Annotated[
        AwareDatetime,
        Field(
            description='The ISO 8601 formatted, UTC-based date and time when the the\nresource will become available.\n\nThis field is intended to indicate to the consumer that they should\nretry their request on or after the listed time to gain an updated\ngateway definition.\n\n**Note:** for the gateway to have a kind of [`Status::Embargoed`],\nby definition, a date at which the resource becomes available _must_\nbe known. If a date is not known, then the resource does not fit the\nAPI\'s definition of "embargoed".'
        ),
    ]
    status: Literal['Embargoed']


class ModelsGatewayClosed4(BaseModel):
    description: Annotated[
        str,
        Field(
            description="A Markdown field (formatted according to the [CommonMark] standard) that\ndescribes the gateway.\n\nAt a minimum, a description of what the gateway is and why it is closed\nis recommended.\n\n**Note:** this field is required for a closed gateway. This was an\nintentional decision: gateways are intended to point users to external\nresources. When that is not possible, a gateway's only purpose can be to\nprovide further information about the data source. Put another way, if\none is attempting to construct a closed gateway with no description, it\nshould be considered why a gateway is needed at all.\n\n[CommonMark]: https://commonmark.org"
        ),
    ]


class ModelsGatewayClosed5(ModelsGatewayClosed1, ModelsGatewayClosed4):
    pass


class ModelsGatewayClosed6(ModelsGatewayClosed2, ModelsGatewayClosed4):
    pass


class ModelsGatewayClosed7(ModelsGatewayClosed3, ModelsGatewayClosed4):
    pass


class ModelsGatewayClosed(
    RootModel[ModelsGatewayClosed5 | ModelsGatewayClosed6 | ModelsGatewayClosed7]
):
    root: Annotated[
        ModelsGatewayClosed5 | ModelsGatewayClosed6 | ModelsGatewayClosed7,
        Field(description='A closed gateway.'),
    ]


class ModelsGatewayLink1(BaseModel):
    url: Annotated[
        str,
        Field(
            description='A uniform resource locator (URL) according to the [URL\nStandard](https://url.spec.whatwg.org/).'
        ),
    ]
    kind: Literal['Direct']


class ModelsGatewayLink2(BaseModel):
    url: Annotated[
        str,
        Field(
            description='A uniform resource locator (URL) according to the [URL\nStandard](https://url.spec.whatwg.org/).'
...
class ModelsGatewayLink3(BaseModel):
    ...
    kind: Literal['Informational']


class ModelsGatewayLink4(BaseModel):
    url: Annotated[
        str,
        Field(
            description='A uniform resource locator (URL) according to the [URL\nStandard](https://url.spec.whatwg.org/).'
    ...
    instructions: Annotated[
        str,
        Field(
            description='The instructions to follow when constructing the email request. At a\nminimum, explaining what criteria are used in determining access,\nwhat information is requested, and what to expect after the\nemail in terms of communication and timeline is recommended.'
        ),
    ]
    kind: Literal['MailTo']


class ModelsGatewayLink(
    RootModel[ModelsGatewayLink1 | ModelsGatewayLink2 | ModelsGatewayLink3 | ModelsGatewayLink4]
):
    root: Annotated[
        ModelsGatewayLink1 | ModelsGatewayLink2 | ModelsGatewayLink3 | ModelsGatewayLink4,
        Field(
...
class ModelsGatewayNamed1(BaseModel):
    link: Annotated[
        ModelsGatewayLink1 | ModelsGatewayLink2 | ModelsGatewayLink3 | ModelsGatewayLink4,
        Field(
            description='A link to an external resource.\n\nA link communicates information about where a resource is located, alongside\nadditional context regarding how the link should be interpreted (via the\n`kind` field). All [`Link`]s include a `url` field pointing to the external\nresource. In the case of [`Link::Approximate`] and [`Link::MailTo`], a\nrequired `instructions` field is included to instruct the user of what steps\nto take after the link has been followed (see the definition of\n[`Link::Approximate`] and [`Link::MailTo`] for more details, respectively).\n\n**Note:** the context of what resources are desired compared with what\nresources the link represents is an important consideration when\nconstructing the correct [`Link`]. For example, if the desired resource is a\nspecific file, but the server can only construct a link to a cohort of\nfiles, a [`Link::Approximate`] should be used. In contrast, if the desired\nresource is the entire cohort of files, a [`Link::Direct`] should be used.\n\n**Note:** the link does not imply the access level or immediate availability\nof the data—it only points a user to where they can _attempt_ to access the\ndata. Instead, [`Link`]s are always wrapped in a [`Gateway`](super::Gateway)\nthat communicates the access level or requirements. In other words, a\n[`Link`] can absolutely require authentication or authorization before data\nbecomes accessible (and the corresponding [`Gateway`](super::Gateway) within\nwhich the [`Link`] is embedded should reflect this).\n\n## Examples\n\n* If the data is contained within a file where a direct link can be\nconstructed, whether that file is open access or controlled, then a\n[`Link::Direct`] should be constructed with a link directly to that file.\n* In the event that study data is deposited as a study within a larger data\nrepository, such as the database of Genotypes and Phenotypes (dbGaP) or\nthe European Genome-phenome Archive (EGA), and the `url` points to the\nstudy page:\n* If the desired resource is a specific file or subset of (but not all)\nfiles within the study, a [`Link::Approximate`] should be returned.\nThis is because the link includes more files than what was\nspecifically requested—thus, instructions on how to filter to the\nfiles requested must be communicated.\n* If the desired resource is _every_ file in the study, then a\n[`Link::Direct`] should be returned pointing to the study page.\n* If the data is not immediately requestable through a webpage but there\nexists an informational page on how to request the data using an\nout-of-band process, then a [`Link::Informational`] should be used.\n* If the data is available after contacting an email address, then a\n[`Link::MailTo`] should be used.',
...
class ModelsGatewayNamed2(BaseModel):
    link: Annotated[
        ModelsGatewayLink1 | ModelsGatewayLink2 | ModelsGatewayLink3 | ModelsGatewayLink4,
        Field(
            description='A link to an external resource.\n\nA link communicates information about where a resource is located, alongside\nadditional context regarding how the link should be interpreted (via the\n`kind` field). All [`Link`]s include a `url` field pointing to the external\nresource. In the case of [`Link::Approximate`] and [`Link::MailTo`], a\nrequired `instructions` field is included to instruct the user of what steps\nto take after the link has been followed (see the definition of\n[`Link::Approximate`] and [`Link::MailTo`] for more details, respectively).\n\n**Note:** the context of what resources are desired compared with what\nresources the link represents is an important consideration when\nconstructing the correct [`Link`]. For example, if the desired resource is a\nspecific file, but the server can only construct a link to a cohort of\nfiles, a [`Link::Approximate`] should be used. In contrast, if the desired\nresource is the entire cohort of files, a [`Link::Direct`] should be used.\n\n**Note:** the link does not imply the access level or immediate availability\nof the data—it only points a user to where they can _attempt_ to access the\ndata. Instead, [`Link`]s are always wrapped in a [`Gateway`](super::Gateway)\nthat communicates the access level or requirements. In other words, a\n[`Link`] can absolutely require authentication or authorization before data\nbecomes accessible (and the corresponding [`Gateway`](super::Gateway) within\nwhich the [`Link`] is embedded should reflect this).\n\n## Examples\n\n* If the data is contained within a file where a direct link can be\nconstructed, whether that file is open access or controlled, then a\n[`Link::Direct`] should be constructed with a link directly to that file.\n* In the event that study data is deposited as a study within a larger data\nrepository, such as the database of Genotypes and Phenotypes (dbGaP) or\nthe European Genome-phenome Archive (EGA), and the `url` points to the\nstudy page:\n* If the desired resource is a specific file or subset of (but not all)\nfiles within the study, a [`Link::Approximate`] should be returned.\nThis is because the link includes more files than what was\nspecifically requested—thus, instructions on how to filter to the\nfiles requested must be communicated.\n* If the desired resource is _every_ file in the study, then a\n[`Link::Direct`] should be returned pointing to the study page.\n* If the data is not immediately requestable through a webpage but there\nexists an informational page on how to request the data using an\nout-of-band process, then a [`Link::Informational`] should be used.\n* If the data is available after contacting an email address, then a\n[`Link::MailTo`] should be used.',
...
class ModelsGatewayNamed3(BaseModel):
    link: Annotated[
        ModelsGatewayLink1 | ModelsGatewayLink2 | ModelsGatewayLink3 | ModelsGatewayLink4,
        Field(
            description='A link to an external resource.\n\nA link communicates information about where a resource is located, alongside\nadditional context regarding how the link should be interpreted (via the\n`kind` field). All [`Link`]s include a `url` field pointing to the external\nresource. In the case of [`Link::Approximate`] and [`Link::MailTo`], a\nrequired `instructions` field is included to instruct the user of what steps\nto take after the link has been followed (see the definition of\n[`Link::Approximate`] and [`Link::MailTo`] for more details, respectively).\n\n**Note:** the context of what resources are desired compared with what\nresources the link represents is an important consideration when\nconstructing the correct [`Link`]. For example, if the desired resource is a\nspecific file, but the server can only construct a link to a cohort of\nfiles, a [`Link::Approximate`] should be used. In contrast, if the desired\nresource is the entire cohort of files, a [`Link::Direct`] should be used.\n\n**Note:** the link does not imply the access level or immediate availability\nof the data—it only points a user to where they can _attempt_ to access the\ndata. Instead, [`Link`]s are always wrapped in a [`Gateway`](super::Gateway)\nthat communicates the access level or requirements. In other words, a\n[`Link`] can absolutely require authentication or authorization before data\nbecomes accessible (and the corresponding [`Gateway`](super::Gateway) within\nwhich the [`Link`] is embedded should reflect this).\n\n## Examples\n\n* If the data is contained within a file where a direct link can be\nconstructed, whether that file is open access or controlled, then a\n[`Link::Direct`] should be constructed with a link directly to that file.\n* In the event that study data is deposited as a study within a larger data\nrepository, such as the database of Genotypes and Phenotypes (dbGaP) or\nthe European Genome-phenome Archive (EGA), and the `url` points to the\nstudy page:\n* If the desired resource is a specific file or subset of (but not all)\nfiles within the study, a [`Link::Approximate`] should be returned.\nThis is because the link includes more files than what was\nspecifically requested—thus, instructions on how to filter to the\nfiles requested must be communicated.\n* If the desired resource is _every_ file in the study, then a\n[`Link::Direct`] should be returned pointing to the study page.\n* If the data is not immediately requestable through a webpage but there\nexists an informational page on how to request the data using an\nout-of-band process, then a [`Link::Informational`] should be used.\n* If the data is available after contacting an email address, then a\n[`Link::MailTo`] should be used.',
...
class ModelsGatewayNamed4(BaseModel):
    kind: Literal['Closed']


class ModelsGatewayNamed5(BaseModel):
    name: Annotated[str, Field(description='The name.')]


class ModelsGatewayNamed6(ModelsGatewayNamed1, ModelsGatewayNamed5):
    pass


class ModelsGatewayNamed7(ModelsGatewayNamed2, ModelsGatewayNamed5):
    pass


class ModelsGatewayNamed8(ModelsGatewayNamed3, ModelsGatewayNamed5):
    pass


class ModelsGatewayNamed9(ModelsGatewayNamed4, ModelsGatewayNamed5):
    pass


class ModelsGatewayClosedStatus1(ModelsGatewayClosed1):
    status: Literal['IndefinitelyClosed']


class ModelsGatewayClosedStatus2(ModelsGatewayClosed2):
    status: Literal['AwaitingPublication']


class ModelsGatewayClosedStatus3(ModelsGatewayClosed3):
    status: Literal['Embargoed']


class ModelsGatewayClosedStatus(
    RootModel[ModelsGatewayClosedStatus1 | ModelsGatewayClosedStatus2 | ModelsGatewayClosedStatus3]
):
    root: Annotated[
        ModelsGatewayClosedStatus1 | ModelsGatewayClosedStatus2 | ModelsGatewayClosedStatus3,
        Field(description='The status of a closed gateway.', discriminator='status'),
    ]


class ModelsMetadataCommonDepositionAccession1(BaseModel):
    kind: Literal['dbGaP']
    value: Annotated[
        str,
        Field(
            description='**`caDSR CDE 11524544 v1.00`**\n\nThis metadata element is defined by the caDSR as "A stable unique\nalphanumeric identifier assigned to a study and any objects by the database\nof Genotypes and Phenotypes (dbGaP).".\n\nLink:\n<https://cadsr.cancer.gov/onedata/dmdirect/NIH/NCI/CO/CDEDD?filter=CDEDD.ITEM_ID=11524544%20and%20ver_nr=1>'
        ),
    ]


class ModelsMetadataCommonDepositionAccession(RootModel[ModelsMetadataCommonDepositionAccession1]):
    root: Annotated[
        ModelsMetadataCommonDepositionAccession1,
        Field(
...
class ModelsMetadataFieldDescriptionUnharmonized(BaseModel):
    harmonized: Annotated[
        bool,
        Field(
            description='Whether or not this field is harmonized across the ecosystem.\n\nThis will always be set to `false`.'
        ),
    ]
    name: Annotated[
        str | None,
        Field(
            description='A display name for this metadata field as _suggested_ by the server (this\nis not considered authoritative and can be ignored by the client if it so\nchooses). This is mainly to avoid naming collisions of common fields across\nservers.'
        ),
    ] = None
    description: Annotated[
        str | None, Field(description='A plain-text description of what the field represents.')
    ] = None
    path: Annotated[
        str,
        Field(
            description="A comma (`.`) delimited path to the field's location on the `metadata`\nobjects returned by the various subject endpoints."
        ),
    ]
    standard: Annotated[
        str | None,
        Field(
            description='If the field is considered harmonized across the federation ecosystem, the\nname of the standard to which the field is harmonized.\n\nIf the field is _not_ harmonized across the federation ecosystem, then this\nshould be [`None`].'
        ),
    ] = None
    url: Annotated[
        str | None,
...
class ModelsMetadataFieldDescriptionHarmonizedStandard(BaseModel):
    name: Annotated[str, Field(description='The name.')]
    url: Annotated[
        str,
        Field(
...
class ModelsSampleIdentifierUnlinkedIdentifier(BaseModel):
    name: str


class ModelsSubjectIdentifierUnlinkedIdentifier(ModelsSampleIdentifierUnlinkedIdentifier):
    pass


class ResponsesByCountValueCount(BaseModel):
    value: Annotated[Any, Field(description='The value.')]
    count: Annotated[int, Field(description='The number of times the value was counted.', ge=0)]


class ResponsesByCountFileResults(BaseModel):
    total: Annotated[int, Field(description='The total number of counts in this result set.', ge=0)]
    missing: Annotated[
        int,
        Field(
            description='The total number of entries that are missing values. In this context,\n"missing" means either (a) the individual metadata key is missing or (b)\nthe entire metadata object is missing.',
            ge=0,
        ),
    ]
    values: Annotated[
        list[ResponsesByCountValueCount],
        Field(description='The counts per value observed for the result set.'),
    ]


class ResponsesByCountSampleResults(ResponsesByCountFileResults):
    pass


class ResponsesByCountSubjectResults(ResponsesByCountFileResults):
    pass


class ResponsesEntityCounts(BaseModel):
    current: Annotated[
        int,
        Field(
    ...
    all: Annotated[
        int, Field(description='The number of entities across all pages in the result set.', ge=0)
    ]


class ResponsesEntitySummary(BaseModel):
    counts: ResponsesEntityCounts


class ResponsesErrorKind1(BaseModel):
    method: Annotated[str, Field(description='The HTTP method that was used in the request.')]
    route: Annotated[str, Field(description='The route that was requested.')]
    kind: Literal['InvalidRoute']


class ResponsesErrorKind2(BaseModel):
    parameters: Annotated[
        list[str] | None,
        Field(
            description='If known, the parameters that are invalid. If not known or the list\nof invalid fields is not desirable to share, pass `null` to this\nfield for a more general error message.'
        ),
    ] = None
    reason: Annotated[
        str, Field(description='A plain-text reason describing why the parameters are invalid.')
    ]
    kind: Literal['InvalidParameters']


class ResponsesErrorKind3(BaseModel):
    entity: Annotated[str, Field(description='The entity (or entities) that are not found.')]
    kind: Literal['NotFound']


class ResponsesErrorKind4(BaseModel):
    entity: Annotated[
        str, Field(description='The entity (or entities) where data cannot be shared.')
    ]
    reason: Annotated[
        str, Field(description='The reason that the line-level data cannot be shared.')
    ]
    kind: Literal['UnshareableData']


class ResponsesErrorKind5(BaseModel):
    field: Annotated[str, Field(description='The field that is not supported.')]
    reason: Annotated[str, Field(description='The reason that the field is not supported.')]
    kind: Literal['UnsupportedField']


class ResponsesErrorKind6(BaseModel):
    message: Annotated[
        str,
        Field(
...
class ResponsesErrorKind7(ResponsesErrorKind1, ResponsesErrorKind6):
    pass


class ResponsesErrorKind8(ResponsesErrorKind2, ResponsesErrorKind6):
    pass


class ResponsesErrorKind9(ResponsesErrorKind3, ResponsesErrorKind6):
    pass


class ResponsesErrorKind10(ResponsesErrorKind4, ResponsesErrorKind6):
    pass


class ResponsesErrorKind11(ResponsesErrorKind5, ResponsesErrorKind6):
    pass


class ResponsesInfoApiInformation(BaseModel):
    api_version: Annotated[
        str,
        Field(description='The version of the API that this server supports.', examples=['v1.3.0']),
    ]
    documentation_url: Annotated[
        str,
        Field(
            description='A URL pointing to the latest version of the Swagger documentation.\n\nNote that, at times, the latest version of the Swagger documentation may\nnot be in sync with the version of the API deployed for this server. The\nintention of this field is not to link to a Swagger specification that\nstrictly matches this particular server, but rather, to point users to\nwhere the specification is developed and hosted.'
        ),
    ]


class ResponsesInfoDataVersion1(BaseModel):
    about: Annotated[
        str,
        Field(
            description='A free-text description of the data version included with the response\nfrom the source server. This field is interpreted as Markdown (as\ndefined by the [CommonMark](https://commonmark.org/) specification).'
        ),
    ]


class ResponsesInfoDataVersion2(BaseModel):
    about_url: Annotated[
        str,
        Field(
            description='A URL where one can learn more about the data versioning for this source\nserver.'
        ),
    ]


class ResponsesInfoDataVersion3(BaseModel):
    version: Annotated[
        int,
        Field(
...
class ResponsesInfoDataVersion4(ResponsesInfoDataVersion1, ResponsesInfoDataVersion3):
    pass


class ResponsesInfoDataVersion5(ResponsesInfoDataVersion2, ResponsesInfoDataVersion3):
    pass


class ResponsesInfoDataVersionAbout1(ResponsesInfoDataVersion1):
    pass


class ResponsesInfoDataVersionAbout2(ResponsesInfoDataVersion2):
    pass


class ResponsesInfoDataVersionAbout(
    RootModel[ResponsesInfoDataVersionAbout1 | ResponsesInfoDataVersionAbout2]
):
    root: Annotated[
        ResponsesInfoDataVersionAbout1 | ResponsesInfoDataVersionAbout2,
        Field(description='A description of how data is versioning within the source server.'),
    ]


class ResponsesInfoServerInformation(BaseModel):
    name: Annotated[
        str | None,
        Field(
            description='The name of this server (if it has one).\n\nThis is a free-text field describing the name of this server, if it has\none. The intention is to be able to describe the proper name of the\napplication.',
    ...
    repository_url: Annotated[
        str | None,
        Field(
            description='If your code base is open source and you want to advertise that, a link\nto the repository where the code is stored.',
            examples=['https://github.com/CBIIT/ccdi-federation-api'],
        ),
    ] = None
    issues_url: Annotated[
        ...
        Field(
            ...
            examples=['https://github.com/CBIIT/ccdi-federation-api/issues'],
        ),
    ] = None


class ResponsesSummaryCounts(BaseModel):
    total: Annotated[int, Field(ge=0)]


class ModelsGateway1(ModelsGatewayNamed1):
    kind: Literal['Open']


class ModelsGateway2(ModelsGatewayNamed2):
    kind: Literal['Registered']


class ModelsGateway3(ModelsGatewayNamed3):
    kind: Literal['Controlled']


class ModelsGateway4(ModelsGatewayNamed4):
    kind: Literal['Closed']


class ModelsGatewayAnonymousOrReference1(BaseModel):
    gateway: Annotated[
        ModelsGateway1 | ModelsGateway2 | ModelsGateway3 | ModelsGateway4,
        Field(
...
class ModelsMetadataFieldDescriptionHarmonized(BaseModel):
    harmonized: Annotated[
        bool,
        Field(
    ...
    standard: ModelsMetadataFieldDescriptionHarmonizedStandard | None = None


class ModelsNamespaceIdentifier(BaseModel):
...
class ModelsSampleIdentifierReferencedIdentifier1(ModelsSampleIdentifierLinkedIdentifier):
    type: Literal['Linked']


class ModelsSampleIdentifierReferencedIdentifier2(ModelsSampleIdentifierUnlinkedIdentifier):
    type: Literal['Unlinked']


class ModelsSubjectIdentifier(BaseModel):
    namespace: ModelsNamespaceIdentifier
    name: Annotated[
        str,
        Field(
            description='**`caDSR CDE 6380049 v1.00`**\n\nThis metadata element is defined by the caDSR as "A unique subject\nidentifier within a site and a study.". No permissible values are defined\nfor this CDE.\n\nLink:\n<https://cadsr.cancer.gov/onedata/dmdirect/NIH/NCI/CO/CDEDD?filter=CDEDD.ITEM_ID=6380049%20and%20ver_nr=1>',
            examples=['SubjectName001'],
        ),
    ]


class ModelsSubjectIdentifierLinkedIdentifier(ModelsSubjectIdentifier):
    server: Annotated[
        str,
        Field(
            description='A uniform resource locator (URL) according to the [URL\nStandard](https://url.spec.whatwg.org/).'
...
class ModelsSubjectIdentifierReferencedIdentifier1(ModelsSubjectIdentifierLinkedIdentifier):
    type: Literal['Linked']


class ModelsSubjectIdentifierReferencedIdentifier2(ModelsSubjectIdentifierUnlinkedIdentifier):
    type: Literal['Unlinked']


class ResponsesErrors(BaseModel):
    errors: Annotated[
        list[
            ResponsesErrorKind7
            | ResponsesErrorKind8
            | ResponsesErrorKind9
            | ResponsesErrorKind10
            | ResponsesErrorKind11
        ],
        Field(description='The errors within this response.'),
    ]
...
class ResponsesInfoDataInformation(BaseModel):
    version: Annotated[
        ResponsesInfoDataVersion4 | ResponsesInfoDataVersion5,
        Field(description='The version of data published within this source server.'),
    ]
    last_updated: Annotated[
        AwareDatetime,
        Field(
            description='The ISO 8601 formatted, UTC-based date and time when the data was last\nupdated.\n\nThis represents the last _update_ time. In contrast to the\n`data_version` field, this field is updated whenever a data update is\nperformed irrespective of whether there were actually changes in the\ndata.'
        ),
    ]
    wiki_url: Annotated[
        str,
        Field(
            description='A URL pointing to the wiki.\n\nThe intention of this field is to make users aware that we maintain a\nfederation-wide wiki that describes the data elements in detail.'
        ),
    ]
    documentation_url: Annotated[
        str | None,
        Field(
            description='If available, a link pointing to where users can learn more about the\ndata contained within this particular server.\n\nThis is intended to be a server-specification documentation link, not\nany link that is developed by the federation.'
        ),
    ] = 'https://docs.example.com'


class FieldOwnedField(BaseModel):
    value: Annotated[Any, Field(description='The value of the metadata field.')]
    ancestors: Annotated[
        list[str] | None,
        Field(
            description='The ancestors from which this field was derived.\n\nAncestors should be provided as period (`.`) delimited paths\nfrom the `metadata` key in the subject response object.'
        ),
    ] = None
    details: ModelsMetadataFieldDetails | None = None
    comment: Annotated[str | None, Field(description='A free-text comment field.')] = None
    owned: Annotated[
        bool | None, Field(description='Whether or not the field is owned by the source server.')
    ] = None


class FieldUnownedField(BaseModel):
    value: Annotated[Any, Field(description='The value of the metadata field.')]
    ancestors: Annotated[
        list[str] | None,
        Field(
...
class FieldUnownedFileChecksums(BaseModel):
...
class FieldUnownedFileType(BaseModel):
    value: Annotated[
        Literal[
            'ADF',
            'AVI',
...
class FieldUnownedSampleAnatomicalSite(BaseModel):
    value: Annotated[
        Literal[
            'anatomical entity',
...
class FieldUnownedSampleDiagnosisCategory(BaseModel):
    value: Annotated[
        Literal[
...
class FieldUnownedSampleDiseasePhase(BaseModel):
    value: Annotated[
        Literal[
            'Post-Mortem',
            'Not Reported',
...
class FieldUnownedSampleLibrarySelectionMethod(BaseModel):
    value: Annotated[
        Literal[
            'Random PCR',
            'PCR',
...
class FieldUnownedSampleLibrarySourceMaterial(BaseModel):
    value: Annotated[
        Literal[
            'Bulk Cells',
            'Bulk Nuclei',
...
class FieldUnownedSampleLibraryStrategy(BaseModel):
    value: Annotated[
        Literal[
            'AMPLICON',
            'ATAC-Seq',
...
class FieldUnownedSamplePreservationMethod(BaseModel):
    value: Annotated[
        Literal[
            '-80 degrees C',
            'Cryopreserved',
...
class FieldUnownedSampleTumorGrade(BaseModel):
    value: Annotated[
        Literal[
            'G1 Low Grade',
...
class FieldUnownedSampleTumorTissueMorphology(BaseModel):
...
class FieldUnownedSubjectAssociatedDiagnoses(BaseModel):
    value: Annotated[
        str,
        Field(
            description="The associated_diagnoses for a [`Subject`](crate::Subject).\n\nThis value can be any permissible diagnosis in v1.7.2 of the CCDI Submission\nTemplate. These values are from the value set **diagnosis_classification**\nfound in the 'Terms and Value Sets' tab from the [CCDI Submission Template\nv1.7.2].\n\nTo facilitate quick access to these values, we have provided a slimmed down\nspreadsheet containing the valid diagnoses:\n\n1. Download the spreadsheet linked below titled\n'CCDI_Submission_Template_v1.7.2.diagnosis_values.xlsx'.\n2. The permissible values are found in column A of the 'diagnosis' tab,\ntitled **diagnosis_category_term**\n\n[CCDI Submission Template v1.7.2]: https://github.com/CBIIT/ccdi-model/blob/682a99d93b66540bb880ce5899ba8096968a96cf/metadata-manifest/CCDI_Submission_Template_v1.7.2.xlsx\n[CCDI_Submission_Template_v1.7.2.diagnosis_values.xlsx]: https://cbiit.github.io/ccdi-federation-api/assets/CCDI_Submission_Template_v1.7.2.diagnosis_values.xlsx"
        ),
    ]
    ancestors: Annotated[
        list[str] | None,
...
class FieldUnownedSubjectAssociatedDiagnosisCategories(BaseModel):
    value: Annotated[
        list[
            Literal[
                'Atypical Teratoid/Rhabdoid Tumors',
...
class FieldUnownedSubjectEthnicity(BaseModel):
    value: Annotated[
        Literal[
...
class FieldUnownedSubjectIdentifier(BaseModel):
    value: Annotated[
        ModelsSubjectIdentifierReferencedIdentifier1 | ModelsSubjectIdentifierReferencedIdentifier2,
        Field(
            description='A referenced identifier for a [`Subject`](crate::Subject).\n\nA referenced identifier is a reference to either an identifier whose owner is known\nand operates an authoritative federation server containing that identifier (i.e., a\n[linked identifier](linked::Identifier)) _or_ a reference to an identifier that is\ngenerally known to be associated with the subject but does not have an associated\nserver that asserts ownership of the identifier (i.e., an [unlinked\nidentifier](unlinked::Identifier)).',
            discriminator='type',
        ),
    ]
    ancestors: Annotated[
...
class FieldUnownedSubjectRace(BaseModel):
    value: Annotated[
        Literal[
            'Not allowed to collect',
...
class ModelsFileIdentifier(BaseModel):
    namespace: ModelsNamespaceIdentifier
    name: Annotated[
        str,
        Field(
            description='**`caDSR CDE 11284037 v1.00`**\n\nThis metadata element is defined by the caDSR as "The literal label for an\nelectronic data file.". No permissible values are defined for this CDE.\n\nLink:\n<https://cadsr.cancer.gov/onedata/dmdirect/NIH/NCI/CO/CDEDD?filter=CDEDD.ITEM_ID=11284037%20and%20ver_nr=1>',
            examples=['File001.txt'],
        ),
    ]


class ModelsFileMetadata(ModelsMetadataCommonMetadata):
    type: FieldUnownedFileType
    size: FieldUnownedFileSize
    checksums: FieldUnownedFileChecksums
    description: FieldUnownedFileDescription
    unharmonized: FieldsUnharmonized | None = None


class ModelsNamespaceMetadata(ModelsMetadataCommonMetadata):
    study_short_title: FieldUnownedNamespaceStudyShortTitle
    study_name: FieldUnownedNamespaceStudyName
    study_funding_id: Annotated[
        list[FieldUnownedNamespaceStudyFundingId], Field(description='The study funding id.')
    ]
    study_id: FieldUnownedNamespaceStudyId
    unharmonized: FieldsUnharmonized | None = None


class ModelsOrganizationMetadata(ModelsMetadataCommonMetadata):
    institution: Annotated[
        list[FieldUnownedOrganizationInstitution],
        Field(
            description='Institutions associated with an organization.\n\n**NOTE:** in this design, an organization in not always a single\ninstitution—it may also represent a consortium of institutions, for\ninstance. This is necessary, since a namespace can be tied to one and\nonly one organization in the API specification. As such, if the above is\nnot true, there is no way to make a namespace where data is contributed\nfrom multiple institutions.'
        ),
    ]
    unharmonized: FieldsUnharmonized | None = None


class ModelsSampleMetadata(ModelsMetadataCommonMetadata):
    age_at_diagnosis: FieldUnownedSampleAgeAtDiagnosis
    anatomical_sites: FieldUnownedSampleAnatomicalSite
    diagnosis: FieldUnownedSampleDiagnosis
    diagnosis_category: FieldUnownedSampleDiagnosisCategory
    disease_phase: FieldUnownedSampleDiseasePhase
    library_selection_method: FieldUnownedSampleLibrarySelectionMethod
    tissue_type: FieldUnownedSampleTissueType
    tumor_classification: FieldUnownedSampleTumorClassification
    tumor_tissue_morphology: FieldUnownedSampleTumorTissueMorphology
    age_at_collection: FieldUnownedSampleAgeAtCollection
    library_strategy: FieldUnownedSampleLibraryStrategy
    library_source_material: FieldUnownedSampleLibrarySourceMaterial
    preservation_method: FieldUnownedSamplePreservationMethod
    tumor_grade: FieldUnownedSampleTumorGrade
    specimen_molecular_analyte_type: FieldUnownedSampleSpecimenMolecularAnalyteType
    identifiers: Annotated[
        list[FieldUnownedSampleIdentifier],
        Field(
            description='The alternate identifiers for the sample.\n\nNote that this list of identifiers *must* include the main identifier\nfor the [`Sample`].'
        ),
    ]
    unharmonized: FieldsUnharmonized | None = None


class ModelsSubjectMetadata(ModelsMetadataCommonMetadata):
    sex: FieldUnownedSubjectSex
    race: Annotated[list[FieldUnownedSubjectRace], Field(description='The race(s) of the subject.')]
    ethnicity: FieldUnownedSubjectEthnicity
    identifiers: Annotated[
        list[FieldUnownedSubjectIdentifier],
        Field(
            description='The alternate identifiers for the subject.\n\nNote that this list of identifiers *must* include the main identifier\nfor the [`Subject`].'
        ),
    ]
    vital_status: FieldUnownedSubjectVitalStatus
    age_at_vital_status: FieldUnownedSubjectAgeAtVitalStatus
    associated_diagnoses: Annotated[
        list[FieldUnownedSubjectAssociatedDiagnoses],
        Field(description='The associated diagnoses for the subject.'),
    ]
    associated_diagnosis_categories: Annotated[
        list[FieldUnownedSubjectAssociatedDiagnosisCategories],
        Field(description='The associated diagnoses categories for the subject.'),
    ]
    unharmonized: FieldsUnharmonized | None = None


class ResponsesInformation(BaseModel):
    server: ResponsesInfoServerInformation
    api: ResponsesInfoApiInformation
    data: ResponsesInfoDataInformation


class ResponsesMetadataFieldDescriptions(BaseModel):
    fields: Annotated[
        list[ModelsMetadataFieldDescriptionHarmonized | ModelsMetadataFieldDescriptionUnharmonized],
        Field(description='Field descriptions.'),
    ]


class FieldUnharmonizedField(RootModel[FieldOwnedField | FieldUnownedField]):
    root: Annotated[FieldOwnedField | FieldUnownedField, Field(description='A metadata field.')]


class ModelsFile(BaseModel):
    id: ModelsFileIdentifier
    samples: Annotated[
        list[ModelsSampleIdentifier],
        Field(
            description='One or more samples that are associated with this [`File`] referred to\nby their identifier(s).\n\nEach file must be associated with at least one\n[`Sample`](super::Sample)—files that are not associated with any sample\nare disallowed. Files may be associated with as many samples as is\nnecessary.\n\n**Note:** each identifier **must** match a [`Sample`](super::Sample)\nthat both (a) is listed in the [`Sample`](super::Sample) index endpoint\n(`/sample`) and (b) is able to be shown with the\n[`Sample`](super::Sample) show endpoint (`/sample/{namespace}/{name}`).'
        ),
    ]
    gateways: Annotated[
        list[
            Annotated[
                ModelsGatewayAnonymousOrReference1 | ModelsGatewayAnonymousOrReference2,
                Field(discriminator='kind'),
            ]
        ]
        | None,
        Field(
            description='One or more [gateways](AnonymousOrReference) through which this file can\nbe accessed.\n\nGateways can be either [anonymous](AnonymousOrReference::Anonymous)\n([gateways](crate::Gateway) with no name) or a\n[refererence](AnonymousOrReference::Reference) to a [named\ngateway](gateway::Named) ([gateways](crate::Gateway) with a name).\n\n**Anonymous** gateways are intended to be embedded directly within a\nreturned file in the `/file` response object. They have no name and are\nonly referred to by the file within which they are embedded.\n\n**Named** gateways, on the other hand, are included in the `gateways`\nkey in the `/file` response object and referred to by name within a\nreturned file in the `/file` response object. They are intended to be\nused when more than one file references the same gateway. This mechanism\nis available to ensure that the gateway object does not need to be\nduplicated multiple times in the response in these cases.\n\nThis field can contain multiple gateways to support scenarios where a\nfile is available through more than one mechanism. We expect that only\none gateway will be returned in most responses.\n\n**Note:** a file must have at least one gateway. If the file has no\ngateway, then it should not be returned as part of this API.'
        ),
    ] = None
    metadata: ModelsFileMetadata | None = None


class ModelsNamespace(BaseModel):
    id: ModelsNamespaceIdentifier
    description: Annotated[
        str,
        Field(
            description='A description of a namespace.\n\nThis description cannot exceed 2048 characters.',
            examples=['A namespace owned by Example Organization.'],
        ),
    ]
    contact_email: Annotated[
        str,
        Field(
            description='A support email address for entities contained within the namespace.\n\nThis field is required to be a valid email address (both in format and\nin terms of the email address being actively monitored).',
            examples=['support@example.com'],
        ),
    ]
    metadata: ModelsNamespaceMetadata | None = None


class ModelsOrganization(BaseModel):
...
class ModelsSubject(BaseModel):
    ...
    gateways: Annotated[
        ...
        | None,
        Field(
            description='One or more [gateways](AnonymousOrReference) through which this subject\ncan be accessed.\n\nGateways can be either [anonymous](AnonymousOrReference::Anonymous)\n([gateways](crate::Gateway) with no name) or a\n[refererence](AnonymousOrReference::Reference) to a [named\ngateway](gateway::Named) ([gateways](crate::Gateway) with a name).\n\n**Anonymous** gateways are intended to be embedded directly within a\nreturned subject in the `/subject` response object. They have no name\nand are only referred to by the subject within which they are embedded.\n\n**Named** gateways, on the other hand, are included in the `gateways`\nkey in the `/subject` response object and referred to by name within a\nreturned subject in the `/subject` response object. They are intended to\nbe used when more than one subject references the same gateway. This\nmechanism is available to ensure that the gateway object does not need\nto be duplicated multiple times in the response in these cases.\n\nThis field can contain multiple gateways to support scenarios where a\nsubject is available through more than one mechanism. We expect that\nonly one gateway will be returned in most responses (if at all).'
        ),
    ] = None
    metadata: ModelsSubjectMetadata | None = None


class ResponsesFile(ModelsFile):
    pass


class ResponsesFiles(BaseModel):
    summary: ResponsesEntitySummary
    data: Annotated[list[ResponsesFile], Field(description='The files.')]
    gateways: (
        list[ModelsGatewayNamed6 | ModelsGatewayNamed7 | ModelsGatewayNamed8 | ModelsGatewayNamed9]
        | None
    ) = None


class ResponsesNamespace(RootModel[ModelsNamespace]):
    root: ModelsNamespace


class ResponsesNamespaces(RootModel[list[ModelsNamespace]]):
    root: Annotated[
        list[ModelsNamespace], Field(description='A response for describing namespaces.')
    ]


class ResponsesOrganization(RootModel[ModelsOrganization]):
    root: ModelsOrganization


class ResponsesOrganizations(RootModel[list[ModelsOrganization]]):
    root: Annotated[
        list[ModelsOrganization], Field(description='A response for describing organizations.')
    ]


class ResponsesSample(ModelsSample):
    pass


class ResponsesSamples(BaseModel):
    summary: ResponsesEntitySummary
    data: Annotated[list[ModelsSample], Field(description='The samples.')]
    gateways: (
        list[ModelsGatewayNamed6 | ModelsGatewayNamed7 | ModelsGatewayNamed8 | ModelsGatewayNamed9]
        | None
    ) = None
...
class ResponsesSubject(ModelsSubject):
    pass


class ResponsesSubjects(BaseModel):
    summary: ResponsesEntitySummary
    data: Annotated[list[ModelsSubject], Field(description='The subjects.')]
    gateways: (
        list[ModelsGatewayNamed6 | ModelsGatewayNamed7 | ModelsGatewayNamed8 | ModelsGatewayNamed9]
        | None
    ) = None
