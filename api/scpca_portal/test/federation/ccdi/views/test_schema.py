...
class SchemaViewTests(SimpleTestCase):
    def test_served_schema_matches_vendored_spec(self):
        response = self.client.get("/federation/ccdi/docs/schema")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.content, VENDORED_SPEC.read_bytes())

    def test_served_schema_is_v1_3_0(self):
        response = self.client.get("/federation/ccdi/docs/schema")
        document = yaml.safe_load(response.content)
        self.assertEqual(document["info"]["version"], "v1.3.0")
