...
class TestListInputPaths(TestCase):
    def setUp(self):
        self.bucket_name = s3.INPUT_BUCKET_NAME
        self.nested_bucket_dirs = self.get_nested_bucket_dirs(self.bucket_name)

    def get_nested_bucket_dirs(self, bucket_name: str) -> str:
        if nested_bucket_path_parts := bucket_name.split("/")[1:]:
            return "/".join(nested_bucket_path_parts)
    ...
    def test_list_input_paths_success(self, mock_run):
        mock_run.return_value = subprocess.CompletedProcess(
            args=["aws", "s3", "ls", f"s3://{self.bucket_name}"],
            returncode=0,
            stdout=f"2024-06-10 10:00:00 1234 {self.nested_bucket_dirs}/file.txt\n"
            f"PRE {self.nested_bucket_dirs}/dir/",
        )
    ...
    def test_list_input_paths_with_relative_path(self, mock_run):
        mock_run.return_value = subprocess.CompletedProcess(
            args=["aws", "s3", "ls", f"s3://{self.bucket_name}/relative/path"],
            returncode=0,
            stdout=f"2024-06-10 10:00:00 1234 {self.nested_bucket_dirs}/file.txt\n"
            f"PRE {self.nested_bucket_dirs}/dir/",
        )
    ...
    def test_list_input_paths_with_nested_directory(self, mock_run):
        mock_run.return_value = subprocess.CompletedProcess(
            args=["aws", "s3", "ls", f"s3://{self.bucket_name}/nested-dir"],
            returncode=0,
            stdout=f"2024-06-10 10:00:00 1234 {self.nested_bucket_dirs}/nested-dir/file.txt\n"
            f"PRE {self.nested_bucket_dirs}/nested-dir/dir/",
        )
