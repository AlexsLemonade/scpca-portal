from django.test import TestCase

from pydantic import ValidationError

from scpca_portal.validators import ProjectData


class ProjectDataValidationTests(TestCase):
    def test_valid_single_cell_list(self):
        data = {
            "includes_bulk": True,
            "single_cell": ["SCPCS999990", "SCPCS999992"],
            "spatial": ["SCPCS999991"],
        }
        validated_data = ProjectData.model_validate(data)

        expected_ids = ["SCPCS000001", "SCPCS000002"]
        self.assertEqual(validated_data.single_cell, expected_ids)

    def test_valid_single_cell_merged_string(self):
        data = {"includes_bulk": False, "single_cell": "MERGED", "spatial": []}
        validated_data = ProjectData.model_validate(data)
        self.assertEqual(validated_data.single_cell, "MERGED")

    def test_invalid_sample_id(self):
        data = {"single_cell": ["INVALID_SAMPLE_ID"]}
        with self.assertRaises(ValidationError) as context:
            ProjectData.model_validate(data)
        self.assertIn("Invalid sample ID", str(context.exception))

    def test_invalid_single_cell_type(self):
        data = {"single_cell": 123}  # not a list or "MERGED" string
        with self.assertRaises(ValidationError) as context:
            ProjectData.model_validate(data)
        self.assertIn("Expected a list of Sample IDs", str(context.exception))

    def test_invalid_spatial_type(self):
        data = {"spatial": 123}  # not a list
        with self.assertRaises(ValidationError) as context:
            ProjectData.model_validate(data)
        self.assertIn("Expected a list of Sample IDs", str(context.exception))

    def test_invalid_includes_bulk_type(self):
        data = {"spatial": 123}  # not a bool
        with self.assertRaises(ValidationError):
            ProjectData.model_validate(data)
