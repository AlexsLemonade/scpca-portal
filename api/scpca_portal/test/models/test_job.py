...
class TestJob(TestCase):
    ...
    def test_bulk_sync_state(self, mock_batch_get_jobs):
        ...
        submitted_jobs = Job.objects.filter(state=JobStates.SUBMITTED)
    ...
    def test_terminate_job(self, mock_batch_terminate_job):
        ...
        terminated_job = JobFactory(
            state=JobStates.TERMINATED, dataset=DatasetFactory(is_processing=False)
        )
    ...
    def test_terminate_job_failure(self, mock_batch_terminate_job):
        job = JobFactory(state=JobStates.SUBMITTED, dataset=DatasetFactory(is_processing=True))
    ...
    def test_terminate_submitted(self, mock_batch_terminate_job):
        ...
        # Should call terminate_job 3 times for submitted, incompleted jobs
    ...
    def test_get_retry_job(self):
        ...
        # Base job's retry_on_termination should update to prevent duplicate retry
        self.assertFalse(
            Job.objects.filter(state=JobStates.TERMINATED).first().retry_on_termination
        )
    ...
    def test_create_terminated_retry_jobs(self):
        ...
        for job in saved_retry_jobs:
            ...
            self.assertFalse(job.retry_on_termination)  # Should update to prevent duplicate retry
        ...
        # Base job's retry_on_termination should update to prevent duplicate retry
        for base_terminated_job in Job.objects.filter(state=JobStates.TERMINATED):
            self.assertFalse(base_terminated_job.retry_on_termination)
