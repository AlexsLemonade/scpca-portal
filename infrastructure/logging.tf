...
resource "aws_cloudwatch_log_stream" "log_stream_api_sync_models" {
  name           = "log-stream-api-sync-models-${var.user}-${var.stage}"
  log_group_name = aws_cloudwatch_log_group.scpca_portal_log_group.name
}

resource "aws_cloudwatch_log_stream" "log_stream_api_sync_metadata" {
  name           = "log-stream-api-sync-metadata-${var.user}-${var.stage}"
  log_group_name = aws_cloudwatch_log_group.scpca_portal_log_group.name
}

resource "aws_cloudwatch_log_stream" "log_stream_api_sync_aggregations" {
  name           = "log-stream-api-sync-aggregations-${var.user}-${var.stage}"
  log_group_name = aws_cloudwatch_log_group.scpca_portal_log_group.name
}
