...
#
# Step 1: Find samples of interest.
...
SAMPLES_RESPONSE=$(curl -s --get \
  ...
  -d "diagnosis=$DIAGNOSIS" # filtering by samples that contain samples with $DIAGNOSIS
)

echo "Found $(echo $SAMPLES_RESPONSE | jq '.count') samples."
...
ALL_COMPUTED_FILES=$(
  echo $SAMPLES_RESPONSE | jq '.results[].computed_files[]'
)
